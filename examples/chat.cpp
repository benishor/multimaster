...
//   ./mm_chat [groupName]
//
// To connect across the internet (WAN), multicast doesn't apply — give each
// node the public host:port of one or more peers as *static peers*. They are
// dialed and the connection is kept up persistently (reconnect with backoff):
//
//   ./mm_chat [groupName] [--port N] [--psk SECRET] [host:port ...]
//
// Pass --psk SECRET (the same value on every node) to encrypt and authenticate
// the mesh: only nodes holding the secret can join, and all traffic is
// encrypted.
//
...
// firewall and, behind NAT, forward it to this host. multimaster does not do
// NAT traversal; one reachable endpoint per pair is enough.
...
// Parse "host:port" (IPv4 or hostname) into a seed_peer. Returns false if
// malformed.
bool parse_endpoint(const std::string& arg, mm::seed_peer& out) {
  ...
  out.port =
      static_cast<uint16_t>(std::strtoul(arg.c_str() + colon + 1, nullptr, 10));
  return out.port != 0;
}
}  // namespace
...
int main(int argc, char** argv) {
  ...
  // Prefer a node's human name (local label or signed nickname) over its hex
  // id.
  ...
  mm::callbacks cb;
  cb.onPeerConnected = [&](mm::peer_id p) {
    std::cout << "[+ connected]    " << who(p) << "\n";
  };
  cb.onPeerDisconnected = [&](mm::peer_id p) {
    std::cout << "[- disconnected] " << who(p) << "\n";
  };
  cb.onMemberJoined = [&](mm::peer_id p) {
    std::cout << "[+ connected]    " << who(p) << "\n";
  };
  cb.onMemberLeft = [&](mm::peer_id p) {
    std::cout << "[- disconnected] " << who(p) << "\n";
  };
  cb.onPeerLost = [&](mm::peer_id p) {
    std::cout << "[x lost]         " << who(p) << "\n";
  };
  cb.onMessage = [&](mm::peer_id from, mm::bytes data) {
    std::cout << who(from) << "> " << to_str(data) << "\n";
  };
  cb.onError = [](const mm::error& e) {
  ...
  std::cout << "this node: " << mesh.id().to_string()
            << "  port: " << mesh.listen_port() << "  group: " << cfg.groupName
            << (cfg.psk.empty() ? "" : "  [secured]") << "\n";
  if (auto pub = mesh.identity_public_key(); !pub.empty()) {
    std::cout << "identity"
              << (cfg.nodeName.empty() ? "" : " \"" + cfg.nodeName + "\"")
              << " pubkey: " << pub << "  (share for peers' --trust)\n";
  }
  if (!cfg.staticPeers.empty()) {
    std::cout << "static peers:";
    for (const auto& s : cfg.staticPeers)
