...
// reaches exactly one, and that stopping a node drives disconnect + lost
// events.
...
TEST("three-node mesh: broadcast, targeted, disconnect, lost") {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    for (int j = 0; j < 3; ++j)
      if (j != i) others.push_back(ports[j]);
  ...
  bool connected = wait_for(
      [&] {
        for (auto& n : nodes)
          if (n->connected.load() < 1) return false;
        return true;
      },
      5000ms);
  CHECK(connected);
  ...
  bool gotTargeted =
      wait_for([&] { return nodes[2]->count_received("TARGET") >= 1; }, 3000ms);
  CHECK(gotTargeted);
  ...
  bool sawDisconnect = wait_for(
      [&] {
        return nodes[0]->disconnects.load() >= 1 ||
               nodes[2]->disconnects.load() >= 1;
      },
      3000ms);
  CHECK(sawDisconnect);
...
// style) peer: discovery is neutralized by giving them distinct multicast
// ports, so a connection proves the static-peer path works. Also verifies
// persistent reconnection after the static peer restarts.
TEST("static peer: connect, message, persistent reconnect") {
  ...
  cfgB.staticPeers.push_back(
      {"127.0.0.1", portA});  // A reachable only as a static peer
  ...
  CHECK(
      wait_for([&] { return node_b->connected_peers().size() >= 1; }, 4000ms));
  ...
  node_a = start_a();
  CHECK(
      wait_for([&] { return node_b->connected_peers().size() >= 1; }, 6000ms));
  CHECK(bConnects.load() >=
        2);  // connected at least twice => reconnect happened
...
// multicast ports; each only dials A as a static peer). Verifies that
// membership gossip relayed through A lets B see C as a *member* (but not a
// direct *peer*), and that B fires on_member_left when C departs.
TEST("membership: non-direct peers visible via gossip; leave on departure") {
  ...
  {
    callbacks cb;
    cb.onPeerConnected = [&, cId](peer_id p) {
      if (p == cId) bDirectToC++;
    };
    cb.onMemberJoined = [&, cId](peer_id p) {
      if (p == cId) bMemberC++;
    };
    cb.onMemberLeft = [&, cId](peer_id p) {
      if (p == cId) bLeftC++;
    };
    node_b->set_callbacks(std::move(cb));
  }
