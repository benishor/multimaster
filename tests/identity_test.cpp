...
// A 32-byte seed filled from a single byte pattern, as hex (for
// identitySeedHex).
...
std::string seed_hex(const crypto::seed32& s) {
...
TEST("identity: derived id, mutual trust, signed name + local label") {
  ...
  cfgA.nodeName = "alice";
  cfgA.trustedKeys.push_back(
      {pubkey_hex(seedB), ""});  // trust B, no local label

  auto cfgB = base_config(47261, {47260});
  cfgB.identitySeedHex = seed_hex(seedB);
  cfgB.nodeName = "bob";
  cfgB.trustedKeys.push_back(
      {pubkey_hex(seedA), "ALICE-LABEL"});  // trust A, with label
...
TEST("identity: untrusted key is refused (revocation)") {
  ...
  // side briefly half-open before the other drops it — revocation is
  // mesh-wide.)
  auto seedA = make_seed(0x11), seedB = make_seed(0x22),
       seedX = make_seed(0x33);
...
TEST("identity: requireIdentity refuses a peer with no identity") {
  ...
  auto cfgB =
      base_config(47267, {47266});  // same PSK, but NO identity configured
...
TEST("identity: name propagates mesh-wide via gossip (B - A - C)") {
  ...
  // ports neutralize discovery). B should still learn C's signed name through
  // A.
  auto seedA = make_seed(0x5A), seedB = make_seed(0x5B),
       seedC = make_seed(0x5C);
  ...
  CHECK(wait_for([&] { return b->node_name(cId) == std::string("node-c"); },
                 6000ms));
