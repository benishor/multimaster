...
// exchange encrypted broadcast/targeted traffic, while a node with the wrong
// PSK is never able to join. Wired via seed peers on 127.0.0.1 (no multicast
// needed).
...
TEST("secured mesh: encrypted broadcast + targeted between PSK peers") {
  ...
  CHECK(wait_for([&] { return nodes[2]->count_received("ENC-TARGET") >= 1; },
                 3000ms));
...
TEST("secured mesh: wrong PSK cannot join") {
  ...
  auto good =
      std::make_unique<mesh>(make_config(portA, {portB}, "the-real-psk"));
  auto bad =
      std::make_unique<mesh>(make_config(portB, {portA}, "an-impostor-psk"));
