...
data_view broadcast_view(const peer_id& src, const message_id& id,
                         std::uint8_t ttl) {
  data_view v;
  v.src = src;
...
TEST("broadcast delivers locally and floods on") {
  ...
  auto v =
      broadcast_view(id_from(1), msg_from(60), 4);  // ttl 4 => should forward
  r.on_data(v, 5, fwd);
...
TEST("ttl exhaustion stops forwarding but still delivers") {
  ...
  auto v = broadcast_view(id_from(1), msg_from(70),
                          1);  // ttl 1 => deliver, no forward
  r.on_data(v, 5, fwd);
  CHECK_EQ(fwd.deliveries, 1);
