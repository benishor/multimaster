...
// Run the two-sided handshake for a given pair of PSKs/groups and report
// whether both sides authenticated each other.
bool handshake_pair(const std::string& pskA, const std::string& pskB,
                    const std::string& group, crypto::secure_session& a,
                    crypto::secure_session& b) {
...
TEST("matching PSK: handshake authenticates and session round-trips") {
  ...
  CHECK(handshake_pair("hunter2-shared-secret", "hunter2-shared-secret", "g", a,
                       b));
...
TEST("counter desync (dropped record) fails to open") {
  ...
  (void)a.seal(std::span<const std::byte>(
      m1.data(), m1.size()));  // advances A's counter, not sent
...
TEST("discovery MAC verifies and rejects tampering / wrong key") {
  ...
  auto other =
      crypto::derive_discovery_key(crypto::derive_group_key("different"));
  ...
  CHECK(crypto::discovery_verify(
      msg, std::span<const std::byte>(tag.data(), tag.size()), dkey));
  // wrong key
  CHECK(!crypto::discovery_verify(
      msg, std::span<const std::byte>(tag.data(), tag.size()), other));
  // tampered message
...
TEST("identity: seed is deterministic, keypair derives stable id") {
  ...
  for (std::size_t i = 0; i < seed.size(); ++i)
    seed[i] = static_cast<std::byte>(i + 1);
...
TEST("identity: sign / verify, with tamper and wrong-key rejection") {
  ...
  CHECK(!crypto::verify_sig(
      std::span<const std::byte>(msg2.data(), msg2.size()), sig, kp.pk));
  // tampered signature
  sig[0] ^= std::byte{0x01};
  CHECK(!crypto::verify_sig(m, sig, kp.pk));
}
