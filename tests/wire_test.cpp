...
TEST("announce round-trip") {
  ...
  auto dec =
      decode_announce(std::span<const std::byte>(enc.data(), enc.size()));
  CHECK(dec.has_value());
...
TEST("announce rejects bad magic") {
  ...
  auto dec =
      decode_announce(std::span<const std::byte>(junk.data(), junk.size()));
  CHECK(!dec.has_value());
}
...
TEST("hello round-trip via frame decode") {
  ...
  auto st =
      try_decode_frame(std::span<const std::byte>(frame.data(), frame.size()),
                       1 << 20, out, consumed);
...
TEST("auth confirm round-trip") {
  ...
  for (std::size_t i = 0; i < tag.size(); ++i)
    tag[i] = static_cast<std::byte>(i * 7 + 1);
  ...
  auto st =
      try_decode_frame(std::span<const std::byte>(frame.data(), frame.size()),
                       1 << 20, out, consumed);
  CHECK_EQ(static_cast<int>(st), static_cast<int>(decode_status::Ok));
  CHECK_EQ(static_cast<int>(out.type),
           static_cast<int>(frame_type::AuthConfirm));
...
TEST("auth confirm with identity signature round-trip") {
  ...
  for (std::size_t i = 0; i < tag.size(); ++i)
    tag[i] = static_cast<std::byte>(i + 3);
  for (std::size_t i = 0; i < sig.size(); ++i)
    sig[i] = static_cast<std::byte>(i * 5 + 2);
...
TEST("secure hello with identity carries id key and name") {
  ...
  h.hasIdentity = true;
  for (std::size_t i = 0; i < h.ephPubKey.size(); ++i)
    h.ephPubKey[i] = static_cast<std::byte>(i);
...
TEST("identity record round-trip") {
  identity_record r;
  for (std::size_t i = 0; i < r.idPubKey.size(); ++i)
    r.idPubKey[i] = static_cast<std::byte>(i);
  r.version = 0x0102030405060708ULL;
  r.name = "sensor-07";
  for (std::size_t i = 0; i < r.signature.size(); ++i)
    r.signature[i] = static_cast<std::byte>(255 - i);
  ...
  CHECK_EQ(static_cast<int>(st), static_cast<int>(decode_status::Ok));
  CHECK_EQ(static_cast<int>(out.type),
           static_cast<int>(frame_type::IdentityRecord));
...
TEST("data round-trip preserves payload") {
  ...
  auto frame =
      encode_data(id_from(1), peer_id{}, mid, 8,
  ...
  CHECK(std::memcmp(out.data.payload.data(), payload.data(), payload.size()) ==
        0);
}
...
TEST("decode reports NeedMore on partial input") {
  ...
  st = try_decode_frame(
      std::span<const std::byte>(frame.data(), frame.size() - 2), 1 << 20, out,
      consumed);
  CHECK_EQ(static_cast<int>(st), static_cast<int>(decode_status::NeedMore));
}
...
TEST("decode rejects oversized frame") {
  ...
  message_id mid{};
  auto frame =
      encode_data(id_from(1), peer_id{}, mid, 4,
                  std::span<const std::byte>(payload.data(), payload.size()));
  parsed_frame out;
  std::size_t consumed = 0;
  auto st =
...
TEST("membership round-trip") {
  ...
  CHECK_EQ(static_cast<int>(st), static_cast<int>(decode_status::Ok));
  CHECK_EQ(static_cast<int>(out.type),
           static_cast<int>(frame_type::Membership));
