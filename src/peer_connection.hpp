...
/// Identity this node advertises to peers in its hello. When `secure` is set
/// the derived `groupKey` / `discoveryKey` authenticate and encrypt the
/// transport and discovery announces respectively.
struct local_identity {
  ...
  crypto::identity_keypair
      idKeys{};          // Ed25519; nodeId is derived from idKeys.pk
...
class connection_listener {
 public:
  ...
  virtual void on_peer_membership(peer_connection& c,
                                  const membership_record& rec) = 0;

  /// An IdentityRecord (signed node-name gossip) frame arrived.
  virtual void on_peer_identity(peer_connection& c,
                                const identity_record& rec) = 0;
...
/// connection_listener::onPeerClosed so the owner can reap after the event
/// batch.
class peer_connection : public io_handler {
 public:
  ...
  [[nodiscard]] std::uint16_t peer_listen_port() const noexcept {
    return peerListenPort_;
  }
  [[nodiscard]] std::uint64_t local_nonce() const noexcept {
    return localNonce_;
  }
  [[nodiscard]] std::uint64_t peer_nonce() const noexcept { return peerNonce_; }
  [[nodiscard]] int fd() const noexcept { return sock_.get(); }
  [[nodiscard]] const sockaddr_in& peer_addr() const noexcept {
    return peerAddr_;
  }
  [[nodiscard]] event_loop::clock::time_point last_recv() const noexcept {
    return lastRecv_;
  }
  [[nodiscard]] event_loop::clock::time_point created_at() const noexcept {
    return createdAt_;
  }
 ...
 private:
  ...
  bool dispatch(parsed_frame& f, std::size_t consumed,
                buffer& src);  // false => dead
  ...
  // Transcript signed/verified for identity: "mm-id-v1" || firstEph ||
  // secondEph || name.
  std::vector<std::byte> id_transcript(
      const std::array<std::byte, 32>& firstEph,
      const std::array<std::byte, 32>& secondEph,
      const std::string& name) const;
  void flush_outbound();
  void enqueue(std::span<const std::byte>
                   frame);  // seal if secure+established, else raw
  void update_epoll();
