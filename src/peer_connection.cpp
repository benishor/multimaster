...
peer_connection::peer_connection(event_loop& loop,
                                 connection_listener& listener,
                                 const mesh_config& cfg,
                                 const local_identity& self, socket sock,
    ...
    : loop_(loop),
      ...
      localNonce_(nonce),
      secure_(self.secure),
      createdAt_(event_loop::clock::now()),
      lastRecv_(createdAt_),
      lastSend_(createdAt_) {
...
peer_connection::peer_connection(event_loop& loop,
    ...
    : loop_(loop),
      ...
      peerAddr_(target),
      inbound_(false),
      state_(conn_state::ConnectingOut),
      localNonce_(nonce),
      secure_(self.secure),
      createdAt_(event_loop::clock::now()),
      lastRecv_(createdAt_),
...
void peer_connection::send_hello() {
  ...
  if (secure_) {
    ephKeys_ =
        crypto::gen_ephemeral();  // fresh per connection => forward secrecy
  ...
  auto frame =
      encode_hello(inbound_ ? frame_type::HelloAck : frame_type::Hello, h);
...
std::vector<std::byte> peer_connection::id_transcript(
    const std::array<std::byte, 32>& firstEph,
    const std::array<std::byte, 32>& secondEph, const std::string& name) const {
  static constexpr char label[] = "mm-id-v1";
  std::vector<std::byte> m;
  m.reserve((sizeof(label) - 1) + firstEph.size() + secondEph.size() +
            name.size());
...
void peer_connection::parse_inbound() {
  ...
  // mesh, once Established the remaining stream is encrypted records, so we
  // stop reading plaintext from in_ and switch to the decrypt path.
...
bool peer_connection::parse_plaintext() {
  for (;;) {
    ...
    decode_status st =
        try_decode_frame(view, cfg_.maxMessageBytes, frame, consumed);
...
bool peer_connection::dispatch(parsed_frame& frame, std::size_t consumed,
                               buffer& src) {
...
bool peer_connection::handle_hello(parsed_frame& frame, std::size_t consumed,
                                   buffer& src) {
  ...
  if (h.protocolVersion != self_.protocolVersion ||
      h.groupName != self_.groupName) {
  ...
  if (!crypto::do_handshake(ephKeys_, h.ephPubKey, self_.groupKey,
                            self_.groupName, hr)) {
...
bool peer_connection::handle_auth_confirm(parsed_frame& frame,
                                          std::size_t consumed, buffer& src) {
  ...
  if (useIdentity_) {
    ...
    if (std::memcmp(derived.data(), peerId_.bytes.data(), derived.size()) !=
        0) {
    ...
    auto msg =
        id_transcript(peerHello_.ephPubKey, ephKeys_.pk, peerHello_.nodeName);
    if (!crypto::verify_sig(std::span<const std::byte>(msg.data(), msg.size()),
                            frame.auth_sig, peerHello_.idPubKey)) {
  ...
  if (out_.size() + rec.size() > cfg_.maxOutboundQueueBytes) {
    switch (cfg_.overflowPolicy) {
      ...
      case mesh_config::overflow::DropOldest:
        ...
        while (!out_.empty() &&
               out_.size() + rec.size() > cfg_.maxOutboundQueueBytes) {
...
void peer_connection::close_gracefully() {
  ...
  enqueue(std::span<const std::byte>(
      bye.data(), bye.size()));  // sealed if secured; flushes
  mark_dead();
}
...
void peer_connection::fail(error_category cat, int err, std::string what) {
  ...
  if (cat != error_category::Socket || err != 0) {
    listener_.report_error(
        {cat, err, std::move(what),
