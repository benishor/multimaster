...
gossip_router::gossip_router(const mesh_config& cfg, event_loop& loop,
                             const peer_id& self)
...
message_id gossip_router::next_id() {
  message_id id;
  std::memcpy(id.bytes.data(), self_.bytes.data(),
              8);  // low 8 bytes: originator
...
void gossip_router::on_data(const data_view& view, int inboundFd,
                            forwarder& fwd) {
  if (seen(view.msgId))
    return;  // duplicate => drop entirely (no deliver, no forward)
  ...
  if (isBroadcast) {
    ...
    if (view.ttl > 1) {
      auto frame =
          encode_data(view.src, view.dst, view.msgId,
                      static_cast<std::uint8_t>(view.ttl - 1), view.payload);
      fwd.forward_except(std::span<const std::byte>(frame.data(), frame.size()),
                         inboundFd);
    }
    return;
  }
...
void gossip_router::originate_targeted(const peer_id& dst, bytes payload,
                                       forwarder& fwd) {
