...
/// direct neighbors}. Every node stores the latest record per origin, builds
/// the undirected adjacency graph (its own direct links + all received
/// records), and takes the set of nodes reachable from itself as the
/// membership. Changes fire onMemberJoined / onMemberLeft; stale records expire
/// so a member whose only bridge died eventually disappears.
///
...
class membership {
 public:
  ...
  /// (-1 = send to all).
  using flood_fn =
      std::function<void(std::span<const std::byte>, int except_fd)>;
  ...
  void set_local_neighbors(std::vector<peer_id> neighbors,
