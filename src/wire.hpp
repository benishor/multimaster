...
/// 128-bit globally-unique-per-message id (originator-derived, no
/// coordination).
struct message_id {
...
/// Announce flag bits.
inline constexpr std::uint8_t kAnnounceFlagSecure =
    0x01;  // sender runs a secured mesh
...
/// Body of a hello / HelloAck frame. When `secure` is set the frame
/// additionally carries the sender's per-connection X25519 ephemeral public
/// key; when `hasIdentity` is set it also carries the sender's long-lived
/// Ed25519 identity public key and self-declared name (proven by the signature
/// in AuthConfirm).
struct hello {
  peer_id nodeId;
  std::uint8_t protocolVersion = 0;
...
/// A decoded application-data frame. `payload` is a view into the caller's
/// input buffer and is only valid until that buffer is consumed/mutated.
struct data_view {
  peer_id src;
...
/// flooded across the mesh so every node can derive the full membership.
/// Ordered per origin by `version` (monotonic; seeded high enough to survive
/// restarts).
...
/// signature is over "mm-idrec-v1" || version (8B big-endian) || name,
/// verifiable against `idPubKey`; the origin's peer_id is the hash of
/// `idPubKey`.
...
struct parsed_frame {
  ...
  std::array<std::byte, 64>
      auth_sig{};            // valid for AuthConfirm when has_auth_sig
...
std::vector<std::byte> encode_auth_confirm(
    const std::array<std::byte, 32>& tag, bool hasSig = false,
