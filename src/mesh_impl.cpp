...
// Decode exactly `n` bytes from 2n hex chars (whitespace trimmed). Returns
// false on any malformed input or length mismatch.
...
bool mesh_impl::load_or_create_seed(crypto::seed32& seed) {
  if (!cfg_.identitySeedHex.empty()) {
    if (!decode_hex(cfg_.identitySeedHex, seed.data(), seed.size())) {
      on_error({error_category::Identity, 0, "invalid identitySeedHex",
                std::nullopt});
      return false;
    }
    return true;
  ...
  if (std::ifstream in(cfg_.identityFile, std::ios::binary); in) {
    ...
    if (!decode_hex(content, seed.data(), seed.size())) {
      on_error({error_category::Identity, 0,
                "malformed identity file: " + cfg_.identityFile, std::nullopt});
      return false;
    }
    return true;
  }
  ...
  if (!out) {
    on_error({error_category::Identity, 0,
              "cannot write identity file: " + cfg_.identityFile,
              std::nullopt});
    return false;
  }
...
void mesh_impl::start() {
  ...
  if (!cfg_.psk.empty()) {
    if (!crypto::init()) {
      on_error({error_category::Crypto, 0,
                "PSK configured but library built without libsodium support",
                std::nullopt});
      return;
    }
    self_.secure = true;
  ...
  const bool identityConfigured =
      !cfg_.identityFile.empty() || !cfg_.identitySeedHex.empty();
  if (identityConfigured) {
    ...
    for (const auto& t : cfg_.trustedKeys) {
      ...
      if (!decode_hex(t.publicKeyHex, pk.data(), pk.size())) {
        on_error({error_category::Identity, 0,
                  "invalid trustedKeys public key: " + t.publicKeyHex,
                  std::nullopt});
        return;
      }
  ...
  listener_ =
      std::make_unique<listener>(loop_, [this](socket s, const sockaddr_in& a) {
  ...
  discovery_ = std::make_unique<discovery>(
      loop_, cfg_, self_,
      [this](const announce& a, const sockaddr_in& src) {
        peers_->on_discovered(a, src);
      },
  ...
  for (auto& c : cmds) {
    switch (c.type) {
      ...
      case command::kind::Targeted:
        peers_->originate_targeted(c.dst,
                                   bytes(c.payload.data(), c.payload.size()));
        break;
...
std::string mesh_impl::node_name(const peer_id& id) const {
  ...
  if (auto it = localLabels_.find(id); it != localLabels_.end())
    return it->second;
...
void mesh_impl::names_snapshot(std::unordered_map<peer_id, std::string> m) {
  auto sp = std::make_shared<const std::unordered_map<peer_id, std::string>>(
      std::move(m));
  std::lock_guard lk(snapMu_);
