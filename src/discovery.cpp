...
discovery::discovery(event_loop& loop, const mesh_config& cfg,
                     const local_identity& self, discover_fn onDiscover,
                     error_fn onError)
    : loop_(loop),
      cfg_(cfg),
      self_(self),
      onDiscover_(std::move(onDiscover)),
...
void discovery::start() {
  auto report = [&](const char* what) {
    if (onError_)
      onError_({error_category::Discovery, errno, what, std::nullopt});
  };

  sock_ = socket::udp();
  if (!sock_.valid()) {
    report("multicast socket()");
    return;
  }
  ...
  if (!make_addr("0.0.0.0", cfg_.multicastPort, bindAddr) ||
      ::bind(sock_.get(), reinterpret_cast<sockaddr*>(&bindAddr),
             sizeof bindAddr) != 0) {
  ...
  ip_mreq mreq{};
  if (::inet_pton(AF_INET, cfg_.multicastAddr.c_str(), &mreq.imr_multiaddr) !=
      1) {
  ...
  if (::setsockopt(sock_.get(), IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
                   sizeof mreq) != 0) {
  ...
  if (!cfg_.multicastIface.empty()) {
    ...
    ::setsockopt(sock_.get(), IPPROTO_IP, IP_MULTICAST_IF, &iface,
                 sizeof iface);
  }
...
void discovery::send_announce() {
  ...
  if (self_.secure) {
    ...
    auto tag = crypto::discovery_tag(
        std::span<const std::byte>(dat.data(), dat.size()), self_.discoveryKey);
...
void discovery::schedule_announce() {
  ...
  std::uniform_int_distribution<long long> jit(-base.count() / 5,
                                               base.count() / 5);
...
void discovery::on_io_events(std::uint32_t events) {
  ...
  for (;;) {
    ...
    std::span<const std::byte> datagram(buf.data(),
    ...
    if (self_.secure) {
      ...
      if (!crypto::discovery_verify(body, received, self_.discoveryKey))
        continue;
    }
