...
peer_manager::peer_manager(event_loop& loop, const mesh_config& cfg,
                           const local_identity& self,
  ...
  // Seed the identity-gossip version from the wall clock so records minted
  // after a restart outrank any still circulating (ordering tag only, like
  // membership).
...
void peer_manager::start() {
  ...
  for (const auto& s : cfg_.staticPeers) {
    static_targets_.push_back(
        static_target{s.host, s.port, std::nullopt, 0, {}, nullptr});
  }
...
void peer_manager::shutdown() {
  running_ = false;
  for (auto& c : conns_) {
    if (c && c->state() == peer_connection::conn_state::Established)
      c->close_gracefully();
  }
...
void peer_manager::maybe_dial(peer_record& r) {
  ...
  if (r.state == peer_state::Connected || r.state == peer_state::Connecting)
    return;
...
void peer_manager::start_dial_addr(const sockaddr_in& addr) {
  auto conn = std::make_unique<peer_connection>(loop_, *this, cfg_, self_, addr,
                                                rand_nonce());
  peer_connection* p = conn.get();
  conns_.push_back(std::move(conn));
...
void peer_manager::accept_connection(socket sock, const sockaddr_in& peerAddr) {
  auto conn = std::make_unique<peer_connection>(
      loop_, *this, cfg_, self_, std::move(sock), peerAddr, rand_nonce());
  conns_.push_back(std::move(conn));
}

// --- connection events ------------------------------------------------------

bool peer_manager::keep_existing(peer_connection* existing,
                                 peer_connection* neu,
  ...
  const bool winnerInbound =
      !weAreLower;  // if peer is lower, it initiated => inbound
...
void peer_manager::on_peer_handshake(peer_connection& c, const hello& peer) {
  ...
  peer_connection* existing =
      (it != estab_.end() && it->second != &c) ? it->second : nullptr;
...
void peer_manager::on_peer_membership(peer_connection& c,
                                      const membership_record& rec) {
  membership_.on_remote_record(rec, c.fd(), event_loop::clock::now());
}

std::vector<std::byte> peer_manager::id_record_msg(
...
identity_record peer_manager::make_self_id_record() {
  ...
  rec.signature = crypto::sign(
      std::span<const std::byte>(msg.data(), msg.size()), self_.idKeys.sk);
  return rec;
}
...
void peer_manager::flood_self_identity(int except_fd) {
  ...
  auto frame = encode_identity(make_self_id_record());
  forward_except(std::span<const std::byte>(frame.data(), frame.size()),
                 except_fd);
  lastIdFlood_ = event_loop::clock::now();
}

void peer_manager::on_peer_identity(peer_connection& c,
                                    const identity_record& rec) {
  if (!self_.hasIdentity)
    return;  // identity gossip only runs in identity meshes
  ...
  auto msg = id_record_msg(rec.version, rec.name);
  if (!crypto::verify_sig(std::span<const std::byte>(msg.data(), msg.size()),
                          rec.signature, rec.idPubKey))
    return;
  ...
  if (auto n = names_.find(origin);
      n == names_.end() || n->second != rec.name) {
...
void peer_manager::forward_except(std::span<const std::byte> frame,
                                  int exceptFd) {
...
bool peer_manager::forward_to(const peer_id& dst,
...
void peer_manager::schedule_reconnect(peer_record& r) {
  ...
  std::uniform_int_distribution<long long> jit(0,
                                               std::max<long long>(1, ms / 5));
  ms += jit(rng_);
  r.reconnectAttempts++;

  peer_id id = r.id;
  loop_.add_timer(std::chrono::milliseconds(ms),
                  [this, id] { on_reconnect_timer(id); });
}
...
void peer_manager::tick() {
  ...
  for (auto& c : conns_) {
    ...
    switch (c->state()) {
      ...
      case peer_connection::conn_state::ConnectingOut:
        if (now - c->created_at() > cfg_.handshakeTimeout)
          c->close_gracefully();
        break;
  ...
  for (auto& [id, r] : records_) {
    ...
    if (r.state == peer_state::Connected || r.state == peer_state::Lost)
      continue;
...
void peer_manager::maintain_static_peers() {
  ...
  for (std::size_t i = 0; i < static_targets_.size(); ++i) {
    ...
    long long cap = cfg_.reconnectMax.count();
    if (ms > cap || ms < 0) ms = cap;
    std::uniform_int_distribution<long long> jit(
        0, std::max<long long>(1, ms / 5));
...
void peer_manager::start_static_dial(std::size_t targetIdx) {
  ...
  if (!resolve_endpoint(t.host, t.port, addr)) {
    delegate_.on_error({error_category::Discovery, 0,
                        "static peer DNS resolution failed: " + t.host,
                        std::nullopt});
