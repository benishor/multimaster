...
/// Acts as the connection_listener for every peer_connection and as the
/// forwarder for the gossip_router.
class peer_manager : public connection_listener, public forwarder {
 public:
  peer_manager(event_loop& loop, const mesh_config& cfg,
               const local_identity& self, peer_manager_delegate& delegate);
  ...
  void originate_broadcast(bytes payload) {
    router_.originate_broadcast(payload, *this);
  }
  ...
  void on_peer_membership(peer_connection& c,
                          const membership_record& rec) override;
  void on_peer_identity(peer_connection& c,
                        const identity_record& rec) override;
  ...
  bool forward_to(const peer_id& dst,
 ...
 private:
  ...
  std::vector<std::byte> id_record_msg(std::uint64_t version,
  ...
  std::unordered_map<peer_id, peer_connection*>
      estab_;  // established, by peer id

  std::vector<static_target> static_targets_;
  std::unordered_map<peer_connection*, std::size_t>
      static_dial_conn_;  // dial -> target idx

  std::unordered_map<peer_id, std::string> names_;  // verified peer names by id
  std::unordered_map<peer_id, std::uint64_t>
      idVersions_;  // identity-gossip dedup
