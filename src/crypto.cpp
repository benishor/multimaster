...
constexpr char kPskCtx[] =
    "multimaster-psk-v1";  // BLAKE2b key for PSK -> master
...
key32 derive_group_key(const std::string& passphrase) {
  ...
  crypto_generichash(uc(out.data()), out.size(),
                     ...
                     passphrase.size(),
                     reinterpret_cast<const unsigned char*>(kPskCtx),
                     sizeof(kPskCtx) - 1);
  return out;
}
...
bool discovery_verify(std::span<const std::byte> msg,
...
bool do_handshake(const ephemeral_keypair& mine, const key32& peerEphPk,
  ...
  if (crypto_scalarmult(uc(dh.data()), uc(mine.sk.data()),
                        uc(peerEphPk.data())) != 0)
    return false;
  ...
  crypto_generichash_init(&st, uc(groupKey.data()), groupKey.size(),
                          master.size());
  ...
  crypto_generichash_update(
      &st, reinterpret_cast<const unsigned char*>(groupName.data()),
      groupName.size());
  ...
  crypto_generichash(uc(skbytes.data()), skbytes.size(), skLabel,
                     sizeof(skLabel) - 1, uc(master.data()), master.size());
  key32 kA{}, kB{};
  ...
  crypto_generichash(uc(confirmBase.data()), confirmBase.size(), cLabel,
                     sizeof(cLabel) - 1, uc(master.data()), master.size());
  crypto_generichash(uc(out.myConfirmTag.data()), out.myConfirmTag.size(),
                     uc(mine.pk.data()), mine.pk.size(), uc(confirmBase.data()),
                     confirmBase.size());
  crypto_generichash(uc(out.expectedPeerTag.data()), out.expectedPeerTag.size(),
                     uc(peerEphPk.data()), peerEphPk.size(),
                     uc(confirmBase.data()), confirmBase.size());
...
std::vector<std::byte> secure_session::seal(
  ...
  crypto_aead_chacha20poly1305_ietf_encrypt(
      uc(rec.data() + 4), &outLen, uc(plaintext.data()), plaintext.size(),
      nullptr, 0, nullptr, nonce, uc(txKey.data()));
  rec.resize(4 + static_cast<std::size_t>(outLen));
  return rec;
}

bool secure_session::open(std::span<const std::byte> ciphertext,
  ...
  if (crypto_aead_chacha20poly1305_ietf_decrypt(
          uc(out.data()), &outLen, nullptr, uc(ciphertext.data()),
          ciphertext.size(), nullptr, 0, nonce, uc(rxKey.data())) != 0)
    return false;
...
std::array<std::byte, 16> id_from_identity(const id_pubkey& pk) {
  ...
  crypto_generichash(uc(out.data()), out.size(), uc(pk.data()), pk.size(),
                     nullptr, 0);
  return out;
}
...
id_sig sign(std::span<const std::byte> msg, const id_seckey& sk) {
  id_sig sig{};
  crypto_sign_detached(uc(sig.data()), nullptr, uc(msg.data()), msg.size(),
                       uc(sk.data()));
  return sig;
}

bool verify_sig(std::span<const std::byte> msg, const id_sig& sig,
                const id_pubkey& pk) {
  return crypto_sign_verify_detached(uc(sig.data()), uc(msg.data()), msg.size(),
                                     uc(pk.data())) == 0;
}

#else  // !MULTIMASTER_HAVE_CRYPTO — safe stubs (a secured mesh refuses to
       // start)
...
bool discovery_verify(std::span<const std::byte>, std::span<const std::byte>,
                      const key32&) {
  return false;
}
ephemeral_keypair gen_ephemeral() { return {}; }
bool verify_tag(std::span<const std::byte>, std::span<const std::byte>) {
  return false;
}
...
std::vector<std::byte> secure_session::seal(std::span<const std::byte>) {
  return {};
}
bool secure_session::open(std::span<const std::byte>, std::vector<std::byte>&) {
  return false;
}
...
bool verify_sig(std::span<const std::byte>, const id_sig&, const id_pubkey&) {
  return false;
}
